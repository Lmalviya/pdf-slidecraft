...
class Settings(BaseSettings):
    ...
    # Processing
    max_parallel_workers: int = Field(default=1, ge=1, le=10, description="Concurrent page workers")
    default_dpi: int = Field(default=200, ge=100, le=400, description="PDF render DPI")
    default_image_quality: int = Field(default=85, ge=50, le=100, description="JPEG quality %")
    max_image_width: int = Field(default=512, ge=256, le=2048, description="Max image dimension for VLM")

    # VLM Reliability
    vlm_timeout_seconds: int = Field(default=300, description="VLM call timeout")
